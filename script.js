...
/*
  ...
  Si aucun champ n'est présent, le rendu reste inchangé.
*/
const PRODUCTS = [
...
(function initEmailJS(){
  if(window.emailjs && typeof window.emailjs.init === 'function'){
    try{ emailjs.init('Q7rxLkalpH60S-9BU'); }catch(e){ /* silent */ }
  }
})();
...
/* Ensure modal/overlay are hidden immediately to prevent initial flash */
...
/* Gestion de la recherche + suggestions */
...
function cartAmount(){ return Object.values(CART).reduce((s,it)=>s + it.qty * it.price,0); }
...
/* Rendu des produits : gère les status (soldout / soon / promo)
   IMPORTANT: badges are now placed inside the image container so they never overlap the product name/price.
   This preserves your existing styles and only adjusts badge placement.
*/
function renderProducts(list){
  ...
  list.forEach(p=>{
    ...
    // determine status from status, status2, status3 in that order
    const s1 = parseStatusField(p.status);
    const s2 = parseStatusField(p.status2);
    const s3 = parseStatusField(p.status3);
    // precedence: soldout > promo > soon (but allow promo even if soon)
    ...
    // If promo, compute new & old price values
    let displayPriceHtml = '';
    if(state && state.type === 'promo' && typeof state.newPrice === 'number'){
    ...
    // visual sold-out style: reduced opacity and non-interactive
    const isSoldOut = state && state.type === 'soldout';

    if (isSoldOut) { card.style.opacity = '0.5'; card.style.pointerEvents = 'none'; }

    // Build innerHTML WITHOUT badges; badges are appended into the .img element afterwards
    card.innerHTML = `
    ...
    // After the card is in the DOM, place badge inside the image to avoid covering name/price
    if(state){
      ...
      if(imgDiv){
        ...
        // Append badge into img container so it sits on top of the image and not the header
        imgDiv.appendChild(badge);
      }
    }

    // If sold out, disable interactive controls (already non-interactive by pointer-events:none),
    // but also detach click handlers to be safe.
    if(!isSoldOut){
      card.querySelector('.incr').addEventListener('click', (e)=>{
        const qEl = card.querySelector('.qty');
        qEl.textContent = parseInt(qEl.textContent || '1') + 1;
      });
      card.querySelector('.decr').addEventListener('click', (e)=>{
        const qEl = card.querySelector('.qty');
      ...
      card.querySelector('.addcart').addEventListener('click', (e)=>{
        ...
        // If promo present, update price temporarily for this cart item to promo price
        let priceToAdd = p.price;
        if(state && state.type === 'promo' && typeof state.newPrice === 'number'){
          priceToAdd = state.newPrice;
        }
      ...
      // visually replace buttons with disabled "Indisponible" for accessibility
...
function addToCart(id,name,price,img,qty){
  if(qty <= 0) return;
  if(!CART[id]) CART[id] = {id,name,price,img,qty};
...
function updateCartUI(){
  ...
  document.getElementById('cartTotal').textContent = formatCurrency(cartAmount());

  // Manage clearCart button state: disable when cart empty
...
orderSend && orderSend.addEventListener('click', async ()=>{
  ...
  try {
    const res = await emailjs.sendForm(serviceID, templateID, orderForm, PUBLIC_KEY);
    loader.close();
    ...
    console.warn('sendForm échoué', sdkErr);
  }
...
/* Petite animation pour le spinner */
...
/* Couleur du texte du hero adaptative selon luminosité du fond (meilleur effort) */
  ...
  try {
    ...
    const onSample = () => {
      try {
        ...
        // CORS ou autre erreur : fallback par défaut
      }
    };
...
/* Assurer que les suggestions restent au-dessus des produits même si le hero est petit */
